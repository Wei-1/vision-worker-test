...
async function theLoop(){
    ...
    if(theResults){
        ...
        if(theResults.faceLandmarks && theResults.faceLandmarks.length > 0){
            canvasCtx.fillStyle = "#CCCCCC";
            Object.keys(FACE_POINTS).forEach(function(key){
                let ll = FACE_POINTS[key];
                for(let i of ll){
                    let p = theResults.faceLandmarks[0][i];
                    canvasCtx.beginPath();
        ...
        if(theResults.poseLandmarks && theResults.poseLandmarks.length > 0){
            canvasCtx.strokeStyle = "#0000CC";
            for(i in POSE_CONNECTIONS){
                let ll = POSE_CONNECTIONS[i];
                let p1 = theResults.poseLandmarks[0][ll[0]];
                let p2 = theResults.poseLandmarks[0][ll[1]];
                connect(p1, p2);
            }
        }
        if(theResults.leftHandLandmarks && theResults.leftHandLandmarks.length > 0){
            ...
            for(i in HAND_CONNECTIONS){
                ...
                let p1 = theResults.rightHandLandmarks[0][ll[0]];
                let p2 = theResults.rightHandLandmarks[0][ll[1]];
                connect(p1, p2);
            }
        }
        if(theResults.leftHandLandmarks && theResults.leftHandLandmarks.length > 0){
            ...
            for(i in HAND_CONNECTIONS){
                let ll = HAND_CONNECTIONS[i];
                let p1 = theResults.leftHandLandmarks[0][ll[0]];
                let p2 = theResults.leftHandLandmarks[0][ll[1]];
                connect(p1, p2);
            }
        }

        if(theResults.poseLandmarks && theResults.poseLandmarks.length > 0) {
            if(theResults.rightHandLandmarks.length > 0){
                canvasCtx.strokeStyle = '#00FF00';
                let p1 = theResults.poseLandmarks[0][POSE_LANDMARKS.RIGHT_ELBOW];
                let p2 = theResults.rightHandLandmarks[0][0];
                connect(p1, p2);
            }
            if(theResults.leftHandLandmarks.length > 0){
                canvasCtx.strokeStyle = '#FF0000';
                let p1 = theResults.poseLandmarks[0][POSE_LANDMARKS.LEFT_ELBOW];
                let p2 = theResults.leftHandLandmarks[0][0];
                connect(p1, p2);
            }
        }
